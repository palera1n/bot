...
from utils.context import CosmoContext
...
async def notify_user_warn(ctx: CosmoContext, target_member: discord.Member, mod: discord.Member, db_user, db_guild, cur_points: int, log):
    ...
    if cur_points >= 600:
        ...
        log_kickban = add_kick_case(
            target_member, mod, "400 or more warn points reached.", db_guild)
...
async def response_log(ctx, log):
    if isinstance(ctx, CosmoContext):
...
async def submit_public_log(ctx: CosmoContext, db_guild: Guild, user: Union[discord.Member, discord.User], log, dmed: bool = None):
