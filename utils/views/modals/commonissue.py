...
class EditCommonIssue(discord.ui.Modal):
    def __init__(self, ctx: CosmoContext, title, issue_message, author: discord.Member) -> None:
        self.ctx = ctx
        self.bot = ctx.bot
        self.author = author
        self.edited = False
            ...
            for component in components:
                if isinstance(component, discord.ActionRow):
                    for child in component.children:
                        buttons.append(
                            (f"{str(child.emoji) + ' ' if child.emoji else ''}{child.label}", child.url))
        ...
        for i in range(2):
            self.add_item(
                discord.ui.TextInput(
                    ...
                    required=False,
                    max_length=80,
                    default=self.buttons[i][0] if len(
                        self.buttons) > i else None
                )
            )
            self.add_item(
                discord.ui.TextInput(
                    ...
                    style=discord.TextStyle.short,
                    required=False,
                    default=self.buttons[i][1] if len(
                        self.buttons) > i else None
                )
            )
