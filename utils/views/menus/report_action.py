...
class WarnView(ui.View):
    def __init__(self, ctx: CosmoContext, member: discord.Member):
    ...
    async def prompt_reason(self, interaction: discord.Interaction):
        view = GenericDescriptionModal(
            self.ctx, interaction.user, title=f"Warn reason for {self.target_member}", label="Why should we warn them?")
