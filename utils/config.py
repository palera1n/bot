...
class Config:
    def __init__(self):
        ...
        if self.markov_enabled != "True":
            ...
            logger.warning(
                "Markov is DISABLED! `/memegen text` features will not be enabled.")

        self.dev = os.environ.get("DEV") is not None

        logger.info(
            f"Cosmo will be running in: {self.guild_id} in \033[1m{'DEVELOPMENT' if self.dev else 'PRODUCTION'}\033[0m mode")
