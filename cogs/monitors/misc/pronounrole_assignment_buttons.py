...
class PronounRoleAssignButtons(commands.Cog):
    ...
    buttons = app_commands.Group(
        name="pronounbuttons", description="Interact with pronoun role buttons", guild_ids=[cfg.guild_id])

    @admin_and_up()
    @buttons.command(description="Post the button role assignment message")
    @transform_context
    async def post_message(self, ctx: CosmoContext):
        ...
        embed = discord.Embed(
            description="Click the buttons to add or remove the pronoun roles of your choice. ", color=discord.Color.blurple())
    ...
    @transform_context
    async def _set(self, ctx: CosmoContext, message_id: str):
        ...
        if message.author != ctx.me:
            raise commands.BadArgument(
                "Message must be sent by me, use `/postembed` or `/postbuttonmessage`")
    ...
    @transform_context
    async def add(self, ctx: CosmoContext, message_id: str):
        try:
            message_id = int(message_id)
        except Exception:
    ...
    async def prompt_for_reaction(self, ctx: CosmoContext, reactions):
    ...
    @transform_context
    async def move(self, ctx: CosmoContext, before: str, after: str):
    ...
    @transform_context
    async def repost(self, ctx: CosmoContext):
        channel = ctx.guild.get_channel(
            guild_service.get_guild().channel_reaction_roles)
...
async def setup(bot):
    await bot.add_cog(PronounRoleAssignButtons(bot))
