...
class Utilities(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.mod_only = ["ModActions", "ModUtils", "Filters", "BoosterEmojis",
                         "RoleAssignButtons", "Giveaway", "Admin", "AntiRaid", "Trivia"]
    ...
    @transform_context
    async def help(self, ctx: CosmoContext, *, command_name: str = None) -> None:
        ...
        if not command_name:
            ...
            for cog_name in self.bot.cogs:
                ...
                for command in cog.__cog_app_commands__:
                    ...
                    command: typing.Union[app_commands.Command,
                                          app_commands.Group] = command
                    spaces_left = ' ' * \
                        (self.left_col_length - len(command.name))
                    if command.description is not None:
                        command.brief = command.description.split("\n")[0]
                    else:
                        command.brief = "No description."
                    cmd_desc = command.brief[0:self.right_col_length] + "..." if len(
                        command.brief) > self.right_col_length else command.brief

                    if isinstance(command, app_commands.Group):
                        string += f"\t* {command.name}{spaces_left} :: {cmd_desc}\n"
                        for c in command.commands:
                            spaces_left = ' ' * \
                                (self.left_col_length - len(c.name)-4)
                            if c.description is not None:
                                c.brief = c.description.split("\n")[0]
                            else:
                                c.brief = "No description."
                            cmd_desc = c.brief[0:self.right_col_length] + "..." if len(
                                c.brief) > self.right_col_length else c.brief
    ...
    @transform_context
    @whisper
    async def usage(self, ctx: CosmoContext, command_name: str):
        command_arg_split = command_name.split()
        if len(command_arg_split) > 1:
            # TODO fix this
            main_command: app_commands.Group = self.bot.tree.get_command(
                command_arg_split[0].lower(), guild=ctx.guild)
            ...
            command = self.bot.tree.get_command(
                command_name.lower(), guild=ctx.guild)
            if not command:
                raise commands.BadArgument("Command not found.")
    ...
    async def get_usage_embed(self,  ctx: CosmoContext, command: app_commands.Command) -> discord.Embed:
        if command.binding.qualified_name in self.mod_only and not gatekeeper.has(ctx.guild, ctx.author, 5):
            raise commands.BadArgument(
                "You don't have permission to view that command.")
            ...
            if command.parent is not None:
                embed = discord.Embed(
                    title=f"/{command.parent.name} {command.name} {args}")
            else:
