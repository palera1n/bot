...
from utils import (CosmoContext, cfg, get_ios_cfw, transform_context,
                   transform_groups)
...
class Devices(commands.Cog):
    ...
    @transform_context
    @always_whisper
    async def add(self, ctx: CosmoContext, device: DeviceTransformer, version: VersionOnDevice) -> None:
    ...
    @transform_context
    @always_whisper
    async def remove(self, ctx: CosmoContext) -> None:
