...
async def handle_support_tag(ctx: CosmoContext, member: discord.Member) -> None:
    if not support_tags:
    ...
    if bucket.update_rate_limit(current) and not (gatekeeper.has(ctx.guild, ctx.author, 5) or ctx.guild.get_role(guild_service.get_guild().role_genius) in ctx.author.roles):
