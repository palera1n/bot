...
class Filters(commands.Cog):
    ...
    @transform_context
    @always_whisper
    async def offlineping(self, ctx: CosmoContext, val: bool = None):
    ...
    _filter = app_commands.Group(
        name="filter", description="Interact with filter", guild_ids=[cfg.guild_id])
    ...
    @transform_context
    async def add(self, ctx: CosmoContext, notify: bool, bypass: int, phrase: str) -> None:
        fw = FilterWord()
        fw.bypass = bypass
        fw.notify = notify
        fw.word = phrase
    ...
    @transform_context
    async def _list(self, ctx: CosmoContext):
    ...
    @transform_context
    async def piracy(self, ctx: CosmoContext, word: str):
        word = word.lower()
    ...
    @app_commands.autocomplete(word=filterwords_autocomplete)
    @transform_context
    async def remove(self, ctx: CosmoContext, word: str):
        word = word.lower()
    ...
    @transform_context
    async def whitelist(self, ctx: CosmoContext, guild_id: str):
        try:
    ...
    @transform_context
    async def blacklist(self, ctx: CosmoContext, guild_id: str):
        try:
            guild_id = int(guild_id)
        except ValueError:
            raise commands.BadArgument("Invalid ID!")
    ...
    @transform_context
    async def ignorechannel(self, ctx: CosmoContext, channel: discord.TextChannel) -> None:
    ...
    @transform_context
    async def unignorechannel(self, ctx: CosmoContext, channel: discord.TextChannel) -> None:
    ...
    @app_commands.describe(channel="Channel to ignore")
    @transform_context
    async def ignorechannellogs(self, ctx: CosmoContext, channel: discord.TextChannel) -> None:
    ...
    @app_commands.describe(channel="Channel to unignore")
    @transform_context
    async def unignorechannellogs(self, ctx: CosmoContext, channel: discord.TextChannel) -> None:
    ...
    @app_commands.autocomplete(word=filterwords_autocomplete)
    @transform_context
    async def falsepositive(self, ctx: CosmoContext, *, word: str):
        word = word.lower()
