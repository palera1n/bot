...
from utils import CosmoContext, cfg, transform_context
...
class ModActions(commands.Cog):
    ...
    @transform_context
    async def warn(self, ctx: CosmoContext, user: ModsAndAboveMemberOrUser, points: app_commands.Range[int, 1, 600], reason: str):
    ...
    @transform_context
    async def kick(self, ctx: CosmoContext, member: ModsAndAboveMember, reason: str) -> None:
    ...
    @app_commands.describe(member="User to kick")
    @transform_context
    async def roblox(self, ctx: CosmoContext, member: ModsAndAboveMember) -> None:
    ...
    @transform_context
    async def mute(self, ctx: CosmoContext, member: ModsAndAboveMember, duration: Duration, reason: str = "No reason.") -> None:
        reason = escape_markdown(reason)
        reason = escape_mentions(reason)
    ...
    @transform_context
    async def unmute(self, ctx: CosmoContext, member: ModsAndAboveMember, reason: str) -> None:
        db_guild = guild_service.get_guild()
    ...
    @transform_context
    async def ban(self, ctx: CosmoContext, user: ModsAndAboveMemberOrUser, reason: str):
        reason = escape_markdown(reason)
        reason = escape_mentions(reason)
        db_guild = guild_service.get_guild()
    ...
    @app_commands.describe(reason="Reason for banning")
    @transform_context
    async def staffban(self, ctx: CosmoContext, user: ModsAndAboveMemberOrUser, reason: str):
        reason = escape_markdown(reason)
        reason = escape_mentions(reason)
        db_guild = guild_service.get_guild()
    ...
    @transform_context
    async def unban(self, ctx: CosmoContext, user: UserOnly, reason: str) -> None:
    ...
    @transform_context
    async def purge(self, ctx: CosmoContext, amount: app_commands.Range[int, 1, 100]) -> None:
        if amount <= 0:
            raise commands.BadArgument(
    ...
    @transform_context
    async def liftwarn(self, ctx: CosmoContext, member: ModsAndAboveMember, case_id: str, reason: str) -> None:
    ...
    @transform_context
    async def editreason(self, ctx: CosmoContext, member: ModsAndAboveMemberOrUser, case_id: str, new_reason: str) -> None:
    ...
    @transform_context
    async def removepoints(self, ctx: CosmoContext, member: ModsAndAboveMember, points: app_commands.Range[int, 1, 600], reason: str) -> None:
        reason = escape_markdown(reason)
        reason = escape_mentions(reason)
