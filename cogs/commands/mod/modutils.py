...
class ModUtils(commands.Cog):
    ...
    @transform_context
    async def rundown(self, ctx: CosmoContext, member: discord.Member):
    ...
    @transform_context
    async def transferprofile(self, ctx: CosmoContext, old_member: ModsAndAboveMemberOrUser, new_member: ModsAndAboveMemberOrUser):
    ...
    @transform_context
    async def clem(self, ctx: CosmoContext, member: discord.Member):
    ...
    @transform_context
    async def freezexp(self, ctx: CosmoContext, member: discord.Member):
        results = user_service.get_user(member.id)
    ...
    @transform_context
    async def birthdayexclude(self, ctx: CosmoContext, member: discord.Member):
        if member.id == self.bot.user.id:
            await ctx.send_error("You can't call that on me :(")
            raise commands.BadArgument("You can't call that on me :(")
    ...
    @transform_context
    async def removebirthday(self, ctx: CosmoContext, member: discord.Member):
        if member.id == self.bot.user.id:
            await ctx.send_error("You can't call that on me :(")
            raise commands.BadArgument("You can't call that on me :(")
    ...
    @transform_context
    async def setbirthday(self, ctx: CosmoContext, member: discord.Member, month: str, date: int):
    ...
    @transform_context
    async def command_ban(self, ctx: CosmoContext, member: ModsAndAboveMember, command_name: str):
        final_command = ""
    ...
    @transform_context
    @always_whisper
    async def say(self, ctx: CosmoContext, message: str, image: ImageAttachment = None, channel: discord.TextChannel = None):
        if channel is None:
            channel = ctx.channel
    ...
    async def prepare_rundown_embed(self, ctx: CosmoContext, user):
    ...
    @transform_context
    async def viewmuted(self, ctx: CosmoContext):
        muted_members = [
