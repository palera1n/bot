...
class Misc(commands.Cog):
    ...
    @transform_context
    @whisper
    async def remindme(self, ctx: CosmoContext, reminder: str, duration: Duration):
    ...
    @transform_context
    async def jumbo(self, ctx: CosmoContext, emoji: str):
    ...
    @transform_context
    @whisper
    async def mybirthday(self, ctx: CosmoContext, month: str, date: int) -> None:
        user = ctx.author
    ...
    @transform_context
    @whisper
    async def avatar(self, ctx: CosmoContext, user: Union[discord.Member, discord.User] = None) -> None:
        if user is None:
            user = ctx.author
    ...
    @transform_context
    async def rule(self, ctx: CosmoContext, title: str, user_to_mention: discord.Member = None):
    ...
    @app_commands.describe(user_to_mention="User to mention in response")
    @transform_context
    async def topic(self, ctx: CosmoContext, channel: discord.TextChannel = None, user_to_mention: discord.Member = None):
    ...
    @transform_context
    async def poll(self, ctx: CosmoContext, question: str, image: ImageAttachment = None, channel: discord.TextChannel = None):
        if channel is None:
            channel = ctx.channel
