...
from utils import CosmoContext, cfg
from utils.context import transform_context
from utils.framework import genius_and_up, whisper_in_general, ImageAttachment, gatekeeper
...
class Genius(commands.Cog):
    ...
    common_issue = app_commands.Group(
        name="commonissue", description="Interact with tags", guild_ids=[cfg.guild_id])
    ...
    @transform_context
    async def new(self, ctx: CosmoContext, title: str,  image: ImageAttachment = None) -> None:
    ...
    @app_commands.describe(image="Image to show in issue")
    @transform_context
    async def edit(self, ctx: CosmoContext, title: str, image: ImageAttachment = None) -> None:
        channel = ctx.guild.get_channel(
            guild_service.get_guild().channel_common_issues)
        if not channel:
            raise commands.BadArgument("common issues channel not found")
    ...
    @transform_context
    async def postembed(self, ctx: CosmoContext, title: str, channel: discord.TextChannel = None, image: ImageAttachment = None):
    ...
    @transform_context
    async def reindex(self, ctx: CosmoContext):
        # get #common-issues channel
    ...
    @transform_context
    async def rawembed(self, ctx: CosmoContext, *, channel: discord.TextChannel, message_id: str, mobile_friendly: bool):
        try:
    ...
    @transform_context
    @whisper_in_general
    async def issue(self, ctx: CosmoContext, title: str, user_to_mention: discord.Member = None):
    ...
    async def solved(self, ctx: CosmoContext):
        ...
        if ctx.author != ctx.channel.owner:  # let OP delete their own thread and geniuses and up
            ...
            if not gatekeeper.has(ctx.guild, ctx.author, 5) and ctx.channel.owner is not None and ctx.channel.owner.top_role >= ctx.guild.me.top_role:
                ...
                raise commands.BadArgument(
                    "Your top role must be higher than the thread owner!")
    ...
    async def on_thread_create(self, thread: discord.Thread):
        ...
        existing_threads_by_author = [t for t in parent.threads if t.owner.id ==
                                      thread.owner.id and not t.archived and t.id != thread.id]
