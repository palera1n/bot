...
from utils import (CosmoContext, cfg, get_ios_cfw, get_ipsw_firmware_info,
...
class iOSCFW(commands.Cog):
    ...
    async def jailbreak(self, ctx: CosmoContext, name: str, user_to_mention: discord.Member = None) -> None:
    ...
    @transform_context
    @whisper_in_general
    async def firmware(self, ctx: CosmoContext, version: str) -> None:
        response = await get_ios_cfw()
        og_version = version
    ...
    @transform_context
    @whisper_in_general
    async def betafirmware(self, ctx: CosmoContext, version: str) -> None:
        response = await get_ios_cfw()
        og_version = version
        for os_version in ["iOS", "tvOS", "watchOS", "audioOS"]:
            version = version.replace(os_version + " ", "")
    ...
    @transform_context
    @whisper_in_general
    async def deviceinfo(self, ctx: CosmoContext, device: str) -> None:
        response = await get_ios_cfw()
    ...
    @transform_context
    @whisper
    async def canijailbreak(self, ctx: CosmoContext, device: DeviceTransformer, version: VersionOnDevice) -> None:
        response = await get_ios_cfw()
        found_jbs = []
    ...
    @transform_context
    @whisper_in_general
    async def bypass(self, ctx: CosmoContext, app: str):
