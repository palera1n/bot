...
from utils import CosmoContext, cfg, format_number, transform_context
...
class Memes(commands.Cog):
    ...
    @transform_context
    async def meme(self, ctx: CosmoContext, name: str, user_to_mention: discord.Member = None):
        name = name.lower()
    ...
    @transform_context
    @whisper
    async def memelist(self, ctx: CosmoContext):
    ...
    @transform_context
    async def add(self, ctx: CosmoContext, name: str, image: ImageAttachment = None) -> None:
        if not name.isalnum():
    ...
    @app_commands.describe(image="Image to show in embed")
    @transform_context
    async def edit(self, ctx: CosmoContext, name: str, image: ImageAttachment = None) -> None:
        if len(name.split()) > 1:
            raise commands.BadArgument(
                "Meme names can't be longer than 1 word.")
    ...
    @app_commands.autocomplete(name=memes_autocomplete)
    @transform_context
    async def delete(self, ctx: CosmoContext, name: str):
        name = name.lower()
    ...
    @transform_context
    @whisper
    async def _8ball(self, ctx: CosmoContext, question: str) -> None:
        responses = ["As I see it, yes.", "Don’t count on it.", "It is certain.", "It is decidedly so.", "Most likely.",
                     "My reply is no.", "My sources say no.", "Outlook not so good.", "Outlook good.", "Signs point to yes.",
    ...
    @transform_context
    async def neuralnet(self, ctx: CosmoContext, image: ImageAttachment) -> None:
    ...
    @transform_context
    async def regular(self, ctx: CosmoContext, top_text: str, bottom_text: str, image: ImageAttachment) -> None:
        if cfg.resnext_token is None:
            raise commands.BadArgument("ResNext token is not set up!")
    ...
    @app_commands.describe(image="Image to use as base")
    @transform_context
    async def motivate(self, ctx: CosmoContext, top_text: str, bottom_text: str, image: ImageAttachment) -> None:
        if cfg.resnext_token is None:
            raise commands.BadArgument("ResNext token is not set up!")
    ...
    @transform_context
    async def aitext(self, ctx: CosmoContext, prompt: str):
    ...
    @transform_context
    @whisper
    async def snipe(self, ctx: CosmoContext):
