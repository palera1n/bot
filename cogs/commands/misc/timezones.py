...
class Timezones(commands.Cog):
    ...
    @transform_context
    @whisper
    async def _set(self, ctx: CosmoContext, zone: str):
    ...
    @transform_context
    @whisper
    async def remove(self, ctx: CosmoContext):
        db_user = user_service.get_user(ctx.author.id)
    ...
    @transform_context
    async def view(self, ctx: CosmoContext, member: discord.Member):
    ...
    @transform_context
    @always_whisper
    async def _list(self, ctx: CosmoContext, country_code: str = None):
        ...
        if country_code is None:
            ...
            if timezones is None:
                raise commands.BadArgument(
                    f"Country code `{country_code}` was not found!")

            timezones.sort()

        menu = Menu(ctx, timezones, per_page=15,
                    page_formatter=format_tz_page, whisper=ctx.whisper)
        await menu.start()
