...
from utils import CosmoContext, canister_search_package, cfg, transform_context
...
class Canister(commands.Cog):
    ...
    @transform_context
    async def package(self, ctx: CosmoContext, query: str) -> None:
        if len(query) < 2:
    ...
    @transform_context
    @whisper_in_general
    async def repo(self, ctx: CosmoContext, query: str) -> None:
