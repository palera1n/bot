...
from utils import CosmoContext, cfg, end_giveaway, transform_context
...
class Giveaway(commands.Cog):
    ...
    giveaway = app_commands.Group(
        name="giveaway", description="Interact with tags", guild_ids=[cfg.guild_id])
    ...
    @transform_context
    async def start(self, ctx: CosmoContext, prize: str, sponsor: discord.Member, time: Duration, winners: int, channel: discord.TextChannel):
        delta = time
        if delta is None:
    ...
    @transform_context
    async def reroll(self, ctx: CosmoContext, message_id: str):
    ...
    @app_commands.describe(message_id="The ID of the giveaway message.")
    @transform_context
    async def end(self, ctx: CosmoContext, message_id: str):
